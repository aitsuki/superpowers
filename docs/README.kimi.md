# Powerset for Kimi Code

Complete guide for using Powerset with [Kimi Code](https://github.com/MoonshotAI/kimi-code).

## Installation

Powerset is available in Kimi Code's plugin marketplace.
...
Go to `Marketplace` > `Powerset` and install it.

You can also install from this repository:

```text
/plugins install https://github.com/aitsuki/powerset
```

For unreleased validation against `dev`, pin the branch explicitly:

```text
/plugins install https://github.com/aitsuki/powerset/tree/dev
```
...
Kimi Code reads Powerset skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Powerset and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info powerset` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Powerset release, install the branch explicitly:
...
1. Confirm `/plugins info powerset` shows the plugin enabled.
