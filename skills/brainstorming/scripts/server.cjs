...
const POWERSET_VERSION = readPowersetVersion();
const TELEMETRY_DISABLE_ENV_VARS = [
  'POWERSET_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const POWERSET_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(POWERSET_VERSION);
  const text = 'Powerset v' + version;
  const logo = '';

  return '<div class="brand"><a href="https://github.com/aitsuki/powerset">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
